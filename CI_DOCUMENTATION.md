...
The CI pipeline is organized into 6 stages with clear dependencies:

1. **Change Detection**: Detects what changed and determines job execution
2. **Quality Checks**: Format, static analysis, pre-commit hooks
3. **Safety Guards**: Policy verification, safety case checks (always run)
4. **Build and Test**: C++ library compilation and testing
5. **Docker Build**: Docker image creation and security scanning
6. **Integration Tests**: ROS2 integration tests
7. **CI Summary**: Comprehensive reporting and status

### Intelligent Job Orchestration

The pipeline uses `dorny/paths-filter@v2` to run jobs only when relevant files change:

- **C++ changes**: include/, src/, tests/, ros2/src/, CMakeLists.txt, *.cmake
- **Docker changes**: Dockerfile, docker-compose.yml, .dockerignore, ros2/**
- **Docs changes**: **/*.md, docs/, markdown/
- **CI changes**: .github/workflows/**, .githooks/**
...
### Cache Types
- **ccache**: Compilation artifact caching (5GB limit)
- **apt**: Package dependency caching
- **CMake**: Build directory caching for incremental builds
- **Docker**: Layer caching with BuildKit optimization
...
- **CI_IMPROVEMENTS.md**: Detailed architecture changes and improvements
- **CI_TESTING_REPORT.md**: Comprehensive testing documentation and validation
- **test_ci_workflow.sh**: Automated validation script (executable)
- **AGENTS.md**: Development workflow and CI guidelines for AI agents
