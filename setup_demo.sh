...
if [ "$USE_DOCKER" = true ]; then
    ...
    echo "1. Full demo (Gazebo + Safety Stack + Nav2 + RViz + Teleop) - Wheel Odometry Navigation"
    echo "2. Safety stack + Simulation (Gazebo + Safety Nodes + RViz + Teleop)"
    echo "3. Safety stack only (no simulation, for physical robot + Teleop)"
